...
bot = commands.Bot(
    ...
    initial_channels=['666_prada']
)


@commands.cooldown(rate=1, per=10)
@bot.command(name='song', aliases=['cjyu','nhtr','vepsrf','gtcyz','track','music','трек','сонг','музыка','ыщтп','екфсл','ьгышс','пiсня'])
async def test(ctx, status=''):
    with vk_api.VkRequestsPool(vk_session) as pool:
        status = pool.method('status.get', {"user_id": "467175732"})
    ...
    if len(status.result) > 1:
        ...
        await ctx.channel.send(f'@{ctx.author.name}, сейчас ничего не играет abs0luteFlex')
